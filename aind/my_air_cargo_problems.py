...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
        ...
        # Create concrete Action objects based on the domain action
        ...
        def load_actions():
            ...
            # the type should not be tested. Is is already assumed
            # Action(Load(c, p, a),
        ...
        def unload_actions():
            ...
            """
            unloads = []
            # create all Unload ground actions from the domain Unload actn
            # the type should not be tested. Is is already assumed
            # Action(Unload(c, p, a),
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        executed.
        """
        # implement (see Russell-Norvig Ed-3 10.2.3)
        # count the number of unsatisfied goals
        count = 0
        node_state = decode_state(node.state, self.state_map).pos
        count = len([goal for goal in self.goal if goal not in node_state])
        return count
